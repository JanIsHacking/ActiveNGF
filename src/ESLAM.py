...
# from src.tsdf_mapper import Mapper
from src.Tracker import Tracker
from src.utils.datasets import get_dataset
from src.utils.Logger import Logger
from src.utils.Mesher import Mesher
from src.utils.Renderer import Renderer, GraspnessRender
...
class ESLAM():
    ...
    def __init__(self, cfg, args, wandb):
        ...
        if cfg['model']['grasp_output']:
            for shared_g_planes in [self.shared_g_planes_xy, self.shared_g_planes_xz, self.shared_g_planes_yz]:
                for i, plane in enumerate(shared_g_planes):
                    plane = plane.to(self.device)
                    plane.share_memory_()
                    shared_g_planes[i] = plane
    ...
    def init_planes(self, cfg):
        ...
        if cfg['model']['grasp_output']:
            g_planes_xy, g_planes_xz, g_planes_yz = [], [], []
            g_planes_res = [self.coarse_planes_res, self.fine_planes_res]
            for grid_res in g_planes_res:
                grid_shape = list(map(int, (xyz_len / grid_res).tolist()))
                grid_shape[0], grid_shape[2] = grid_shape[2], grid_shape[0]
                g_planes_xy.append(torch.empty([1, planes_dim, *grid_shape[1:]]).normal_(mean=0, std=0.01))
                g_planes_xz.append(torch.empty([1, planes_dim, grid_shape[0], grid_shape[2]]).normal_(mean=0, std=0.01))
                g_planes_yz.append(torch.empty([1, planes_dim, *grid_shape[:2]]).normal_(mean=0, std=0.01))

            self.shared_g_planes_xy = g_planes_xy
            self.shared_g_planes_xz = g_planes_xz
            self.shared_g_planes_yz = g_planes_yz
